...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ...
  I1= (1/12)*m1*(l1^2);
  I2= (1/2)*m2*(wr^2);
  a = ((m1*(l1^2))/4)+ I1 + (m2*(l1^2)) + I2;
  b = (m1*g*(l1/2))+(m2*g*l1);
  dy(1,1) = y(2);
  dy(2,1) = ((b*y(1))- u)/(a-I2);
  ##dy(2,1) = ((((m1*g*(l1/2))+(m2*g*l1))*y(1))- u)/((((m1*(l1^2))/4)+ I1 + (m2*(l1^2)) + I2)-I2);
  dy(3,1) = y(4);
  dy(4,1) = ((b*y(1)) - ((a/I2)*u))/(I2-a);
  ##dy(4,1) = ((((m1*g*(l1/2))+(m2*g*l1))*y(1)) - (((((m1*(l1^2))/4)+ I1 + (m2*(l1^2)) + I2)/I2)*u))/(I2-(((m1*(l1^2))/4)+ I1 + (m2*(l1^2)) + I2));
  ##disp(dy);
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  ...
  a = ((m1*(l1^2))/4) + I1 + (m2*(l1^2)) + I2;
  b = (m1*g*(l1/2))+(m2*g*l1);
  A = [ 0 1 0 0;(b/(-(I2-a))) 0 0 0;0 0 0 1; (b/(I2-a)) 0 0 0];
  B = [0;-1/(a-I2);0;-a/(I2*(I2-a))]; 
...
function [t,y] = pole_place_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  ...
  eigs = [ -1; -5; -10; -15]; ## Initialise desired eigenvalues
  K = place(A, B, eigs);    ## Calculate K matrix for desired eigenvalue
  disp((K));
  disp(100*(norm(A)/norm(B)));
  tspan = 0:0.1:10; 
...
function [t,y] = lqr_RW_pendulum(m1, m2, l1, wr, g, y_setpoint, y0)
  ...
  [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g);               ## Initialize A and B matrix
  Q = [10 0 0 0; 0 10 0 0;0 0 1 0; 0 0 0 10];                   ## Initialise Q matrix
  R = [1];                   ## Initialise R 
  K = lqr(A, B, Q, R);
  disp("A:")
  disp(A);
...
function RW_pendulum_main()
  ...
  y0 = [pi*0.9; 0; 0; 0]; # Initial Conditions
